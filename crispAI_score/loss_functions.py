...
def scvi_log_zinb_positive_complex(x, mu, theta, pi, class_label_prob0, class_label_prob1, eps=1e-8):
    """
    https://github.com/YosefLab/scVI/blob/6c9f43e3332e728831b174c1c1f0c9127b77cba0/scvi/models/log_likelihood.py#L206
    """
    ...
    '''
    Modify loss such that for samples with class_label_prob < 0.5 (i.e. not in the class),
    we do not penalize the model. Those samples are masked out.
    '''
    ...
    # class_label_prob0 is 0 class probability
    # class_label_prob1 is 1 class probability
    mask = class_label_prob1 > class_label_prob0
    ...
    # if res is nan or empty insert 0 in res
    if torch.isnan(res).any() or res.shape[0] == 0:
        res = torch.zeros(1).to(res.device)
    

    return res.mean()


class MyZeroInflatedNegativeBinomialLoss_complex(nn.Module):
    """
    ZINB loss, based on scvi
    """
    ...
    def forward(self, preds, target, class_label_prob0, class_label_prob1):
        #preds, theta, pi = preds[:3]
        preds_ = preds
        preds = preds_[:, 0]
        theta = preds_[:, 1]
        ...
        l = -scvi_log_zinb_positive_complex(target, preds, theta, pi, class_label_prob0, class_label_prob1)
...
def zero_inflated_negative_binom_loss(
    ...
    nb_loss_func = negative_binom_loss(
        mean=False, eps=eps, scale_factor=scale_factor, debug=debug, tb=None
    )
